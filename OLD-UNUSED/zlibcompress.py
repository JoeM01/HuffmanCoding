...
#REMINDER - ZLIB is DEFLATE, utilizing huffman and LZ77

def zlib_compress(input_file, compressed_file):
...
def zlib_decompress(compressed_file, decompressed_file):
...
def zlib_compressor(input_file = None, compressed_File = None, decompressed_file = None, automated = False): #to call in main
    try:
        ...
        if automated:
            ...
            compressed_file = f"compression_outputs\zlib_{file_name}_compressed.bin"
            decompressed_file = f"compression_outputs\zlib_{file_name}_decompressed.txt"

        compressed = zlib_compress(input_file, compressed_file)
        ...
        decompressed = zlib_decompress(compressed_file, decompressed_file)
